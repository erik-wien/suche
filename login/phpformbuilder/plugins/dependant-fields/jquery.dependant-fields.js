/*=========================================================
            phpformbuilder dependant fields for jQuery
...
(function($)
{
    $.fn.dependantFields = function(options){
        var settings = $.extend({
            // defaults settings.
        ...
        $(this).each(function() {
            var parentField = $(this).attr(settings.parentData),
                showValues  = $(this).attr(settings.showValuesData).split(' '),
            ...
            $('*[name^="' + parentField + '"]').on('change ifChanged changed.bs.select', function() {
                ...
                if($.inArray(value, showValues) > -1) {
                    ...
                    if(inverseData === true) {
                        ...
                        $(that).find('input, textarea, select').prop('disabled', 'disabled');
                    } else {

                        // Show
                        $(that).removeClass(settings.hideClass).addClass(settings.showClass);
                        $(that).find('input, textarea, select').prop('disabled', false);
                    }
                } else {

                    // if value NOT found in showValues
                    if(inverseData === true) {
                        if($.inArray($(this).val(), showValues) > -1) {

                            // if checked value is show value => return
                            return;
                        } else {
                            if(value === undefined) {
                                ...
                                if(othersChecked === false) {
                                    ...
                                    // If none ckecked except showValues => Hide
                                    $(that).removeClass(settings.showClass).addClass(settings.hideClass);
                                    $(that).find('input, textarea, select').prop('disabled', 'disabled');
                                }
                            } else {
            ...
            $('*[name="' + parentField + '"]:first').trigger('change');
        });
    };
